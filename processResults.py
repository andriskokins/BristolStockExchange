...
import numpy as np  # Often useful with plotting

# --- Apply Professional Plotting Styles (similar to the box plot) ---
sns.set_theme(style="whitegrid")  # Seaborn style
plt.rcParams.update({
    "font.family": "serif",  # Ensure 'serif' or a similar font like 'Arial', 'Calibri' is available
    "axes.titlesize": 18,
    "axes.labelsize": 15,
    "xtick.labelsize": 13,
    "ytick.labelsize": 13,
    "legend.fontsize": 12,  # Adjusted for line plot legends
    "figure.titlesize": 20  # For suptitle if used
})
...
if (num_total_cols - first_agent_col_index) < 0:
    print(
        f"ProcessResults WARNING: Not enough columns for any agent data. Expected at least {first_agent_col_index + 4} columns, got {num_total_cols}.")
elif (num_total_cols - first_agent_col_index) % 4 != 0:
    print(
        f"ProcessResults WARNING: Number of columns for agent data ({num_total_cols - first_agent_col_index}) is not a multiple of 4. Check CSV structure.")
...
for i in range(num_agent_blocks):
    ...
    if agent_avg_balance_data_col_idx < num_total_cols:
        ...
        if pd.isna(agent_name) or str(agent_name).strip() == "":
            print(
                f"ProcessResults WARNING: Agent name at original column index {agent_name_source_col_idx} is empty or NaN. Skipping this agent.")
            continue
...
for agent_col_name in agent_avg_balance_cols_to_plot:
    if agent_col_name in avgBalance.columns:
        ...
        print(
            f"ProcessResults WARNING: Column '{agent_col_name}' not found for numeric conversion. It might have been skipped.")
